...
if (!isset($_GET['id'])) {
   redirect_to(url_for('public/blog/index.php'));
}
$id = $_GET['id'];
...
if (is_post_request()) {
   ...
   // logfile
   log_action('Delete blog', "id: {$blog->id}, Deleted by {$loggedInAdmin->full_name()}", "admin");

   // Delete agents
   $result = $blog->deleted($id);
   $session->message('The blog was deleted successfully.');
   redirect_to(url_for('public/blog/index.php'));
} else {
   // Display form
}
...
<div class="row">
   <div class="card text-center p-4">
      ...
      <p class="item"><?php echo h($blog->title); ?></p>

      <form action="<?php echo url_for('public/blog/delete.php?id=' . h(u($id))); ?>" method="post">
         <div id="operations" class="btn-group">
            <input type="submit" name="commit" class="btn btn-sm btn-danger border-0" value="Yes" />
            <a href="<?php echo url_for('public/blog/index.php'); ?>" class="btn btn-sm btn-dark">No</a>
         </div>
      </form>
   </div>
